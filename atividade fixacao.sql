...
#Exercicios
update Conta_Corrente set valorLimite_cc = valorLimite_cc + (valorLimite_cc * 0.50) where (valorLimite_cc < 200) and (taxa_mensal_cc >= 2) and (taxa_mensal_cc <= 10) and (dataAbertura_cc < '2014-01-01');

#1 Crie um update que mude o nome do cliente nº5 para ‘LucasMatos’;
...
update Cliente set renda_cli  = renda_cli + (renda_cli * 20 / 100) where(dataNasc_cli < '2000-01-01');

#3 Crie um update para diminuir a renda em 27% dos clientes homens com renda maior ou igual a 5000 reais;
update Cliente set renda_cli = renda_cli - (renda_cli * 27 / 100) where(sexo_cli = 'M') and (renda_cli >= 5000);
...
update Conta_corrente set taxa_mensal_cc = taxa_mensal_cc + (taxa_mensal_cc * 30 /100) where(saldo_cc >= 500) and (dataAbertura_cc >= '2009-01-01') and (dataAbertura_cc <= '2009-06-30');
